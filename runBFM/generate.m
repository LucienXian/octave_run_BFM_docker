...
%% load raw BFM models
load('/source/raw/01_MorphableModel.mat');

%% load 3ddfa data
% 1. load bfm information. trim 
load('/source/3ddfa/model_info.mat');
...
% 2. load expression
load('/source/3ddfa/Model_Expression.mat');
model.expMU = mu_exp;
model.expPC = w_exp;
model.expEV = sigma_exp;

% 3. load mouth tri
load('/source/3ddfa/Model_tri_mouth');
model.tri_mouth = tri_mouth;
model_info.tri_mouth = tri_mouth;

% 4. face contour 
load('/source/3ddfa/Model_face_contour_trimed.mat');
...
% 5. nose hole
load('/source/3ddfa/Modelplus_nose_hole.mat');
...
load('/source/3ddfa/Modelplus_parallel')
model_info.parallel = parallel;
model_info.parallel_face_contour = parallel_face_contour;

% 7. pncc
copyfile('/source/3ddfa/vertex_code.mat', 'Out/pncc_code.mat')

%% load 3DMMasSTN UV coords
load('/source/stn/BFM_UV.mat');
...
clear -x model model_info UV

save -mat '/source/Out/BFM.mat' model
save -mat '/source/Out/BFM_info.mat' model_info
save -mat '/source/Out/BFM_UV.mat' UV